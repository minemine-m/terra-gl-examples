...
// ... (rest of the script)References to features
...
<template>
  <div class="relative w-full h-full">
    ...
    <div class="absolute top-4 right-4 bg-gray-800 p-4 rounded-lg shadow-lg z-10 w-64 border border-gray-700">
      <h3 class="text-white font-bold mb-4 border-b border-gray-600 pb-2">{{ t('examples.geometryStyles.panelTitle') }}</h3>
      
      <!-- Line Controls -->
      <div class="mb-6">
        <h4 class="text-cyan-400 font-bold text-sm mb-2">{{ t('examples.geometryStyles.cyanLine') }}</h4>
        <div class="mb-2">
          <label class="block text-gray-400 text-xs mb-1">{{ t('examples.geometryStyles.width') }}: {{ controls.lineWidth }}px</label>
          <input v-model.number="controls.lineWidth" type="range" min="1" max="20" class="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer">
        </div>
        <div>
          <label class="block text-gray-400 text-xs mb-1">{{ t('examples.geometryStyles.color') }}</label>
          <input v-model="controls.lineColor" type="color" class="w-full h-6 bg-transparent cursor-pointer rounded">
        </div>
      </div>

      <!-- Polygon Controls -->
      <div>
        <h4 class="text-green-400 font-bold text-sm mb-2">{{ t('examples.geometryStyles.greenPolygon') }}</h4>
        <div class="mb-2">
          <label class="block text-gray-400 text-xs mb-1">{{ t('examples.geometryStyles.opacity') }}: {{ controls.polyOpacity }}</label>
          <input v-model.number="controls.polyOpacity" type="range" min="0" max="1" step="0.1" class="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer">
        </div>
        <div class="mb-2">
           <label class="block text-gray-400 text-xs mb-1">{{ t('examples.geometryStyles.borderWidth') }}: {{ controls.polyBorderWidth }}px</label>
          <input v-model.number="controls.polyBorderWidth" type="range" min="0" max="10" class="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer">
        </div>
        <div>
          <label class="block text-gray-400 text-xs mb-1">{{ t('examples.geometryStyles.color') }}</label>
          <input v-model="controls.polyColor" type="color" class="w-full h-6 bg-transparent cursor-pointer rounded">
        </div>
      </div>
    </div>
  </div>
